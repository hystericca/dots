...
// Local macOS tuning over sahaj-b/ghostty-cursor-shaders cursor_warp.glsl.
// Keeps the upstream Neovide-like shape, but makes the animation faster and
// quieter for Mina Lowlight.
...
const float DURATION = 0.08; // total animation time
const float TRAIL_SIZE = 0.62; // 0.0 = all corners move together. 1.0 = max smear (leading corners jump instantly)
const float THRESHOLD_MIN_DISTANCE = 1.15; // min distance to show trail (units of cursor height)
...
const float FADE_EXPONENT = 3.2; // exponent for fade gradient along the trail
